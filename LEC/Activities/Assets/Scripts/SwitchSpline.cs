...
public class SwitchSpline : MonoBehaviour
{
    ...
    public float speed = 1.0f, distToKnot = 0.01f;
    ...
    private void Update()
    {
        ...
        if (isDoLerp)
        {
            ...
            if (!isInitSpline)
            {
                transform.position = Vector3.Lerp(endKnot0Pos, starKnot1Pos, lerpTime);
            }

            if (isInitSpline)
            {
                transform.position = Vector3.Lerp(endKnot1Pos, starKnot0Pos, lerpTime);
            }
        }
        */

        // if is on spline 0, mov on spline 0 (initial spline), else move on spline 1 (second spline)
        //if (!isDoLerp)
        //{
        if (isInitSpline)
        {
